...
public class PaymentController {
	...
	public ResponseEntity<String> createPaymentOrder(@RequestBody Map<String, Object> requestBody, HttpServletRequest request) {
		try {
			...
			//Extract totalAmount and cartItem from the request body
			BigDecimal totalAmount = new BigDecimal(requestBody.get("totalAmount").toString());
			List<Map<String, Object>> cartItemRaw = (List<Map<String, Object>>) requestBody.get("cartItems");
			
			//Convert cartItemsRaw to list<OrderItem>
			List<OrderItem> cartItems = cartItemRaw.stream().map(item -> {
			...
			//call the payment sevice to create a Razorpay order
